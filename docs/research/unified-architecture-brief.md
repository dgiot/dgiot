...
| `dgiot_collector/src/core/dgiot_adapter.py` | dgiot 平台双通道适配器（REST + MQTT） |
| `dgiot_collector/src/storage/tdengine_manager.py` | TDengine 批量写入器（5s 缓冲） |
| `dgiot_collector/src/config/endpoints.yaml` | 9 种接入场景端点配置 |
| `dgiot_collector/src/config/thing_models/G1-G8_standard.yaml` | 8 组标准行业物模型（80+ 测点） |
