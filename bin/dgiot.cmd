...
@set dgiot_conf=%etc_dir%\dgiot.conf
...
:: Extract node name from dgiot.conf
@for /f "usebackq delims=\= tokens=2" %%I in (`findstr /b node\.name "%dgiot_conf%"`) do @(
  @call :set_trim node_name %%I
)

:: Extract node cookie from dgiot.conf
@for /f "usebackq delims=\= tokens=2" %%I in (`findstr /b node\.cookie "%dgiot_conf%"`) do @(
...
:generate_app_config
@set gen_config_cmd=%escript% %cuttlefish% -i %rel_dir%\dgiot.schema -c %etc_dir%\dgiot.conf -d %data_dir%\configs generate
...
@set description=DGIOT node %node_name% in %rootdir%
@if "" == "%2" (
  %erlsrv% add %service_name% %node_type% "%node_name%" -on restart -c "%description%" ^
           -i "dgiot" -w "%rootdir%" -m %erl_exe% -args %args% ^
           -st "init:stop()."
  sc config dgiot start=delayed-auto
) else (
  :: relup and reldown
  goto relup
)
