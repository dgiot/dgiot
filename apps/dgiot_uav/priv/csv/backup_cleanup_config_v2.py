...
if not os.path.exists(JSON_DIR):
    print(f"错误: JSON_DIR不存在: {JSON_DIR}")
    sys.exit(1)
if not os.path.exists(CSV_DIR):
    print(f"错误: CSV_DIR不存在: {CSV_DIR}")
    sys.exit(1)
if not os.path.exists(CONFIG_DIR):
    print(f"错误: CONFIG_DIR不存在: {CONFIG_DIR}")
    sys.exit(1)
...
FILES_TO_BACKUP = {
    'json': [
        ...
        '1600.json',
        '1700.json',
        'test_items_summary.json.backup',
        'test_items_summary_new.json',
    ],
}
...
def analyze_config_files():
    ...
    for filename, size in sorted(json_files):
        status = "✅ 保留" if filename in KEEP_FILES else "📦 备份"
...
def main():
    ...
    backup_count = sum([
        len([f for f in FILES_TO_BACKUP['json'] if os.path.exists(os.path.join(JSON_DIR, f))]),
