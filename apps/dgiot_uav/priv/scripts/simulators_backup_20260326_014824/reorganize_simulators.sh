...
# 确认操作
read -p "⚠️  即将整理simulators目录，是否继续？(yes/no): " confirm
if [ "$confirm" != "yes" ]; then
    echo "❌ 操作已取消"
    exit 0
fi
...
if [ -f "check_magnetic_environment.py" ]; then
    mv check_magnetic_environment.py tools/check_environment.py
    echo "  ✅ tools/check_environment.py"
fi
...
echo ""
echo "总计核心文件: $(find stations devices core tools docs -maxdepth 1 -name '*.py' -o -name '*.md' | wc -l) 个"
echo "----------------------------------------"
echo ""
