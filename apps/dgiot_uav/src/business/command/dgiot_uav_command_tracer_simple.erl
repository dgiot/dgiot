...
%%% @end
%%%-------------------------------------------------------------------
-module(dgiot_uav_command_tracer_simple).
-author("johnliu").
...
%% ETS表定义
-define(TABLE_COMMAND_TRACES, uav_command_traces_simple).
...
trace_command_flow(CommandType, StationId, TargetAddr, CommandCode, Params) ->
    io:format("【链路跟踪】开始跟踪命令流程: Type=~p, StationId=~p, TargetAddr=~p, Code=~p~n", 
    ...
    io:format("【链路跟踪】跟踪ID创建: ~s~n", [TraceId]),
    {ok, TraceId}.
...
trace_command_result(TraceId, Status, Result, Details) ->
    case get_trace(TraceId) of
        {ok, Trace} ->
            ...
            io:format("【链路跟踪】命令结果: TraceId=~s, Status=~p, Result=~p~n", 
                      [TraceId, Status, Result]),
            ok;
            
        {error, not_found} ->
            io:format("【链路跟踪】未找到跟踪记录: ~s~n", [TraceId]),
            ok
    end.
...
log_command_start(CommandType, StationId, TargetAddr, CommandCode, Params) ->
    case trace_command_flow(CommandType, StationId, TargetAddr, CommandCode, Params) of
        {ok, TraceId} ->
            io:format("【命令开始】Type=~p, StationId=~p, TargetAddr=~p, Code=~p, Value=~p, TraceId=~s~n", 
                      [CommandType, StationId, TargetAddr, CommandCode, 
                       maps:get(value, Params, 0), TraceId]),
            TraceId;
        {error, Reason} ->
            io:format("【命令开始】跟踪创建失败: ~p~n", [Reason]),
            %% 返回一个虚拟的TraceId，避免后续函数调用失败
            <<"trace_failed_", (integer_to_binary(erlang:system_time(millisecond)))/binary>>
    end.
...
log_command_send(TraceId, NodeType, NodeName, DataSize, ChannelType, Details) ->
    case get_trace(TraceId) of
        {ok, Trace} ->
            ...
            io:format("【命令发送】TraceId=~s, Node=~p:~p, Size=~p, Channel=~p~n", 
                      [TraceId, NodeType, NodeName, DataSize, ChannelType]),
            ok;
            
        {error, not_found} ->
            io:format("【命令发送】未找到跟踪记录: ~s~n", [TraceId]),
            ok
    end.
...
log_command_receive(TraceId, NodeType, NodeName, DataSize, Details) ->
    case get_trace(TraceId) of
        {ok, Trace} ->
            ...
            io:format("【命令接收】TraceId=~s, Node=~p:~p, Size=~p~n", 
                      [TraceId, NodeType, NodeName, DataSize]),
            ok;
            
        {error, not_found} ->
            io:format("【命令接收】未找到跟踪记录: ~s~n", [TraceId]),
            ok
    end.
...
log_command_complete(TraceId, NodeType, NodeName, Duration, Details) ->
    case get_trace(TraceId) of
        {ok, Trace} ->
            ...
            io:format("【命令完成】TraceId=~s, Node=~p:~p, Duration=~pms~n", 
                      [TraceId, NodeType, NodeName, Duration]),
            ok;
            
        {error, not_found} ->
            io:format("【命令完成】未找到跟踪记录: ~s~n", [TraceId]),
            ok
    end.
...
log_command_error(TraceId, NodeType, NodeName, Error, Duration, Details) ->
    case get_trace(TraceId) of
        {ok, Trace} ->
            ...
            io:format("【命令错误】TraceId=~s, Node=~p:~p, Error=~p, Duration=~pms~n", 
                      [TraceId, NodeType, NodeName, Error, Duration]),
            ok;
            
        {error, not_found} ->
            io:format("【命令错误】未找到跟踪记录: ~s~n", [TraceId]),
            ok
    end.
...
test_plc_command_flow() ->
    ...
    io:format("【测试】PLC命令流程测试完成，TraceId=~s~n", [TraceId]),
    ok.

%% @doc 测试治具命令流程
test_fixture_command_flow() ->
    io:format("【测试】开始测试治具命令流程~n"),
    ...
    io:format("【测试】治具命令流程测试完成，TraceId=~s~n", [TraceId]),
    ok.

%% @doc 测试无人机命令流程
test_uav_command_flow() ->
    io:format("【测试】开始测试无人机命令流程~n"),
    ...
    io:format("【测试】无人机命令流程测试完成，TraceId=~s~n", [TraceId]),
    ok.

%% @doc 测试所有命令流程
test_all_flows() ->
    io:format("【测试】开始测试所有命令流程~n"),
    ...
    case get_all_traces() of
        {ok, Traces} ->
            io:format("【测试】找到 ~p 条跟踪记录:~n", [length(Traces)]),
            lists:foreach(fun(Trace) ->
                io:format("  Trace: ~p~n", [Trace])
            end, Traces),
            io:format("【测试】总计 ~p 条跟踪记录~n", [length(Traces)]);
        {error, Reason} ->
            io:format("【测试】获取跟踪记录失败: ~p~n", [Reason])
    end,
    
    io:format("【测试】所有命令流程测试完成~n"),
    ok.
    ...
    case ets:info(?TABLE_COMMAND_TRACES) of
        undefined ->
            ets:new(?TABLE_COMMAND_TRACES, [
                ...
                {keypos, #command_trace.trace_id},  % 使用trace_id作为键
                {write_concurrency, true},
                {read_concurrency, true}
            ]),
            io:format("【链路跟踪】ETS表创建成功: ~p~n", [?TABLE_COMMAND_TRACES]);
        _ ->
            io:format("【链路跟踪】ETS表已存在: ~p~n", [?TABLE_COMMAND_TRACES])
    end,
    ok.
...
store_trace(Trace) ->
    init_ets(),
    ets:insert(?TABLE_COMMAND_TRACES, Trace),
    ok.
...
start() ->
    init_ets(),
    io:format("【链路跟踪】命令跟踪器已启动~n"),
    ok.

%% @doc 停止函数（模块清理）
stop() ->
    ets:delete(?TABLE_COMMAND_TRACES),
    io:format("【链路跟踪】命令跟踪器已停止~n"),
    ok.