...
%%    dgiot_http_server:bind(<<"/swagger_hjt212.json">>, ?MODULE, [], priv)
swagger_hjt212() ->
    [
        dgiot_http_server:bind(<<"/swagger_hjt212.json">>, ?MODULE, [], priv)
    ].
...
do_request(get_decoder, #{<<"buff">> := Buff}, _Context, _Req) ->
    dgiot_hjt212_decoder:parse_frame(dgiot_utils:hex_to_binary(Buff), #{});
...
do_request(post_encoder, Body, _Context, _Req) ->
    R = dgiot_hjt212_decoder:to_frame(Body),
