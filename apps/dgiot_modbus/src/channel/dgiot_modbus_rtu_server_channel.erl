...
%%--------------------------------------------------------------------
-module(dgiot_modbus_rtu_server_channel).
...
init(?TYPE, ChannelId, Args) ->
    ...
    try
        ...
        ?LOG(info, "Channel initialized: Type=~p, ProductId=~p, App=~p, Header=~p, Len=~p, Port=~p", 
             [Type, ProductId, App, Header, Len, Port]),
        
        {ok, State, dgiot_modbus_rtu_server:start(Port, State)}
    catch
        error:{badmatch, _} ->
