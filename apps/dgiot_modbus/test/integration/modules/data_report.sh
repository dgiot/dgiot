...
check_data_parsing() {
    ...
    case "$result" in
        "has_block_data")
            log_success "数据解析检查通过（包含block_data）"
            return 0
            ;;
        "no_cache")
            log_error "数据解析检查失败（无缓存数据）"
            return 1
            ;;
        "no_block_data")
            log_error "数据解析检查失败（无block_data字段）"
            return 1
            ;;
        *)
            log_error "数据解析检查失败（未知状态: $result）"
            return 1
            ;;
    esac
}
