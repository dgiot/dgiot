        ...
        {ok, Channel} ->
            ...
            case dgiot_parse:get_object(<<"Device">>, DeviceId, [{"X-Parse-Session-Token", SessionToken}], [{from, rest}]) of
                {ok, #{<<"objectId">> := DeviceId, <<"product">> := #{<<"objectId">> := ProductId}}} ->
                    dgiot_tdengine:get_tddata(Channel, ProductId, DeviceId, Args);
                _ ->
                ...
                {ok, #{<<"results">> := Result} = All} ->
                    NewResult =
                        lists:foldl(fun(X, Acc) ->
                            ...
                            case Body of
                                #{<<"include">> := <<"product">>} ->
                                    dgiot_tdengine:get_tddata(Channel, Acc, X, Product, DevAddr);
                                _ ->
                                    dgiot_tdengine:get_tddata(Channel, Acc, maps:without([<<"product">>], X), Product, DevAddr)
                            end
                                    end, [], Result),
                    {200, All#{<<"results">> => NewResult}};
                {error, What} ->
