...
for i in ${tables[@]}; do
  echo $i
  psql -U postgres -d parse -c "COPY (SELECT * FROM \"$i\" where _rperm && array['*','role:$1'])  TO '/home/postgres/$1/$i.csv' WITH  CSV HEADER;"
...
for i in ${relations[@]}; do
  echo $i
  psql -U postgres -d parse -c "COPY \"$i\" TO '/home/postgres/$1/relations/$i.csv' WITH  CSV HEADER;"
