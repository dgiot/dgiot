...
ipv6_probe(Opts) ->
    case persistent_term:get({?MODULE, ipv6_probe_supported}, unknown) of
        unknown ->
            ...
            OtpVsn = dgiot_vm:get_otp_version(),
...
-spec(check_oom(dgiot_types:oom_policy()) -> ok | {shutdown, term()}).
check_oom(Policy) ->
    check_oom(self(), Policy).

-spec(check_oom(pid(), dgiot_types:oom_policy()) -> ok | {shutdown, term()}).
...
%% Get Proc's Stats.
-spec(proc_stats() -> dgiot_types:stats()).
proc_stats() -> proc_stats(self()).

-spec(proc_stats(pid()) -> dgiot_types:stats()).
proc_stats(Pid) ->
