...
-define(TAB, dgiot_channel_registry).
...
is_enabled() ->
    dgiot:get_env(enable_session_registry, true).

%% @doc Register a global channel.
-spec(register_channel(binary() | {binary(), pid()}) -> ok).
...
-spec(unregister_channel(binary()| {binary(), pid()}) -> ok).
unregister_channel(ChannelId) when is_binary(ChannelId) ->
    unregister_channel({ChannelId, self()});
...
-spec(lookup_channels(binary()) -> list(pid())).
lookup_channels(ChannelId) ->
    [ChanPid || #channel{pid = ChanPid} <- mnesia:dirty_read(?TAB, ChannelId)].
