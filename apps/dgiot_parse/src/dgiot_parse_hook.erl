...
%% API
-export([do_request_hook/6,
         subscribe/3, subscribe/4,
         unsubscribe/3, unsubscribe/4,
         publish/2,
         add_trigger/3,
         del_trigger/2, del_trigger/1, del_trigger/0,
         update_trigger/3,
         get_trigger/0, get_trigger/2,
         add_all_trigger/1,
         do_hook/2, do_hook/1,
         notify/6,
         api_hook/1]).
...
del_trigger(Name, Class) ->
    lists:foreach(
      ...
      [<<"beforeSave">>, <<"beforeDelete">>, <<"afterSave">>, <<"afterDelete">>]).
    ...
    case get_trigger(Name) of
        {ok, Triggers} ->
            ...
            case dgiot_parse:get_schemas(Name, <<>>) of
                {ok, #{<<"results">> := Results}} ->
                    Fun =
                        fun(#{<<"className">> := Class}, Acc) when Class == <<"_Session">> ->
                                Acc;
                           (#{<<"className">> := Class}, Acc) ->
                                lists:foldl(
