...
- Issues: [![GitHub issues](https://img.shields.io/github/issues/priestjim/gen_rpc.svg)](https://hub.fastgit.org/priestjim/gen_rpc/issues)
...
```erlang
{deps, [
    {gen_rpc, {git, "https://hub.fastgit.org/priestjim/gen_rpc.git", {branch, "master"}}}
]}.
```
...
`gen_rpc` uses [hut](https://hub.fastgit.org/tolbrino/hut) for logging. This allows the developer to integrate the logging library of their choice by providing the appropriate definition in their `rebar.config`. The default logging facility of `hut` is SASL.

For more information on how to enable `gen_rpc` to use your own logging facility, consult the [README.md](https://hub.fastgit.org/tolbrino/hut#supported-logging-backends) of `hut`.
...
Enter `erlang:spawn/4` (_remote spawn_ from now on). Remote spawn dynamically spawns processes on a remote node, skipping the single-mailbox restriction that `rex` has. The are various libraries written to leverage that loophole (such as [Rexi](https://hub.fastgit.org/cloudant/rexi)), however there's a catch.
...
- [Edward Tsang](https://hub.fastgit.org/linearregression)
