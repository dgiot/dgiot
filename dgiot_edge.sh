...
function build_dashboard() {
  ...
  ${script_dir}/node-v16.15.1-linux-x64/bin/pnpm install --no-frozen-lockfile
  ${script_dir}/node-v16.15.1-linux-x64/bin/pnpm build
  echo "build_dashboard"
}

function build_dgiot_web() {
  install_node

  cd ${script_dir}/dgiot_web
  git reset --hard
  git pull

  rm ${script_dir}/dgiot_web/build/ -rf
  ${script_dir}/node-v16.15.1-linux-x64/bin/npm install
  ${script_dir}/node-v16.15.1-linux-x64/bin/npm build
  echo "build_dgiot_web"
}
...
function pre_build_dgiot() {
    ...
    mkdir -p  ${script_dir}/$plugin/apps/dgiot_api/priv/www/
    rm ${script_dir}/$plugin/rebar.config  -rf
    cp ${script_dir}/$plugin/apps/dgiot/conf/rebar.config ${script_dir}/$plugin/rebar.config  -rf
    rm ${script_dir}/$plugin/rebar.config.erl  -rf
    cp ${script_dir}/$plugin/apps/dgiot/conf/rebar.config.erl ${script_dir}/$plugin/rebar.config.erl  -rf
    rm ${script_dir}/$plugin/data/loaded_plugins.tmpl -rf
    cp ${script_dir}/$plugin/apps/dgiot/conf/loaded_plugins.tmpl ${script_dir}/$plugin/data/loaded_plugins.tmpl
    rm ${script_dir}/$plugin/apps/dgiot_parse/etc/dgiot_parse.conf -rf
    cp ${script_dir}/$plugin/apps/dgiot/conf/dgiot_parse.conf ${script_dir}/$plugin/apps/dgiot_parse/etc/dgiot_parse.conf -rf

    rm ${script_dir}/$plugin/apps/dgiot_http/etc/dgiot_http.conf -rf
    cp ${script_dir}/$plugin/apps/dgiot/conf/dgiot_http.conf ${script_dir}/$plugin/apps/dgiot_http/etc/dgiot_http.conf -rf
...
function post_build_dgiot() {
    ...
    mv ./${software}.tar.gz ${install_dir}/go_fastdfs/files/package/
  }
...
function install_node() {
   ...
   if [ -d ${script_dir}/node-v16.15.1-linux-x64/bin/ ]; then
      ...
      ${script_dir}/node-v16.15.1-linux-x64/bin/npm i -g pnpm --registry=https://registry.npmmirror.com
      ${script_dir}/node-v16.15.1-linux-x64/bin/pnpm config set registry https://registry.npmmirror.com
      ${script_dir}/node-v16.15.1-linux-x64/bin/pnpm add -g pnpm to update
      ${script_dir}/node-v16.15.1-linux-x64/bin/pnpm -v
      ${script_dir}/node-v16.15.1-linux-x64/bin/pnpm get registry
      echo -e "`date +%F_%T` $LINENO: ${GREEN}  ${script_dir}/node-v16.15.1-linux-x64/bin/node/ ${NC}"
